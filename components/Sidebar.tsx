...
const Sidebar = () => {
  const { currentUser, logout, currentView, setCurrentView } = useAppStore();

  const menuItems = [
    { id: 'orders', label: 'Running Orders', icon: ShoppingCart },
    { id: 'payments', label: 'Payments (M-Pesa)', icon: CreditCard },
    { id: 'clients', label: 'Clients', icon: Users },
    { id: 'freight', label: 'Set Freight', icon: Container, adminOnly: false },
    { id: 'stock', label: 'Stock Taking', icon: ClipboardCheck, adminOnly: true },
  ...
  return (
    <div className="w-64 bg-slate-900 text-slate-300 h-screen flex flex-col fixed left-0 top-0 shadow-xl z-20">
      <div className="p-6 border-b border-slate-700">
        ...
        <p className="text-xs text-slate-500 mt-1">v2.7.0 • {currentUser?.role}</p>
      </div>
